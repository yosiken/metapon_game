...
「浮上中に2連鎖を繋ぐのが自分で楽しいか」を判定するためのもの。
演出は最小限だが、**ブロック・氷・背景にはチップ素材を当ててある**。
成功の体感と上下方向の読み取りやすさが素材の有無で変わり、それが P0 の
問いへの答えを左右するため。素材の選定根拠は `asetts/chips/README.md`。
...
> **ティックレートは 78Hz**（BGM の BPM 130 に対し、拍グリッドが整数ティックに乗る最小の
> 60Hz超の値。仕様書 §4.6.3）。描画は通常どおり 60fps。
> BPM を変える場合は `scripts/sim/cfg.gd` の `BPM` と `TICKS` を、
> `TICKS * 15 / BPM` が整数になる組み合わせに直すこと。
...
スマホにはキーボードが無いため、**一時停止 / リトライ / 情報** は画面下部のボタンから、
埋没時のリトライは画面のどこをタップしても行える。
画面右上（Lv. 表示の下）に `fps / sim Hz` を常時表示しており、sim が目標の 78Hz を
